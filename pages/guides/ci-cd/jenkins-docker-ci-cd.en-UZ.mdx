...
**7->** Yuklab olgan Service accoun  private keyimizni Jenkins credentialsga qo'shib qo'yamiz.

**->** `Manage Jenkins` **->** `Credentials`-ga kirib  `Add credentials`ga o'tamiz.

![docker-ci-cd](/images/tutorials/ci-cd/jenkins-docker/gcp6.png)

**8->** Google Clouddan yuklab olgan service accoun private keyimizni ochib ko'rsak 2chi qatorida **project_id** ko'rsatilgan bo'ladi uni nusxalab olib uni ham Jenkins credentialsga qo'shib qo'yamiz.
...
echo 'y' | docker container prune
cat $REGISTRY_TOKEN | docker login -u _json_key --password-stdin https://gcr.io
docker pull $IMAGE
echo "Clean temp Container"
        ...
        script {
            sshagent(credentials: ['server-ssh']) {
                sh """
                    ...
                    --registry-host=${REGISTRY_URL} \
                    --container-name=${CONTAINER_NAME} \
                    --registry-token=${GCR_CREDENTIALS}
                """
            }
        }   
    }
