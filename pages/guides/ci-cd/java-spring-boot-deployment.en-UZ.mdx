...
Ushbu amaliyotda quyidagi Java Spring Boot application kodlaridan foydalanamiz **Gitlab->** [**gitlab.com/ismoilovdev/waifulist**](https://gitlab.com/ismoilovdev/waifulist), **Github->** [**github.com/devops-journey-uz/waifulist**](https://github.com/devops-journey-uz/waifulist)
</Callout>
...
management.endpoint.health.show-details=always
```
Bu xavsfzilik uchun yaxshi yechim emas chunki secretlar ochiq holda turibdi buni **Gitlab CI variable** yoki **Github Secrets**ga qo'shib ishlatishni ko'rib chiqamiz bu amaliyotda.
...
yuqorida buyruq ikkita key generatsiya qiladi public(`id_rsa.pub`) va private(`id_rsa`) keylar, bu keylar `~/.ssh` papkasiga saqlanadi.
...
`SERVER_USERNAME` nomli variablega serverga kirish uchun user nomini yozamiz, userni `whoami` buyrug'ini ishga tushirib olishimiz mumkin.
```bash
whoami
```
...
`SERVER_IP` nomli variablega server ip manzilini(Github bo'glana oladigan IP) yozamiz.
