...
![nginx-lb](/images/tutorials/web-server/nginx-lb/main-banner.png)
...
![nginx-lb](/images/tutorials/web-server/nginx-lb/nginx-main.png)
...
1. Dockerni bash script yozib o'rntaamiz.
...
Docker o'rnatilganidan keyin SonarQubeni ishga tushiramiz. SonarQube default `9000` ishga tushadi

```bash
docker run -d --name sonar -p 9000:9000 sonarqube:lts-community
```

SonarQube 3ta application serverimizda ham ulanganidan keyin har biriga kirib **Login**ga `admin` **Passowrdg**a `admin` yozib kiramiz va yangi parol qo'yihsda quyidagicha parol qo'yamiz.
...
Bunday ko'rinishda login parol qo'yishnik o'ziga xos xususiyati bor u orqali biz load balancer qaysi serverdagi SonarQubega yo'naltirganini bilib olishimiz va Load Balancing metodlari to'gri ishlayotganini bilib olamiz.
...
Bu NGINX konfiguratsiya devops-hourney.uz kelgan requestlarni 3ta applciation serverga Load balancing qilib turishi kerak.
...
* `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;` Clientning IP manzilini mavjud `X-Forwarded-For` headriiga qo'shib, mavjud qiymatlarni saqlaydi. Ushbu header asl clientning IP-manzilini kuzatish uchun ishlatiladi, ayniqsa so'rovlar proksi-serverlar yoki load balancerlar orqali o'tganda.
...
SSL sertifikat olib HTTP HTTPS qilish uchun certbot o'rnatib SSL sertifikat olamiz
...
Brauzerdan applicationimizga domen orqali kiramiz va maximum 3ta urinish bilan parolni topib login qilib kira olamiz. Bu safar bu muvaffaqqiytali bo'ldi yana biz login qilib kira oldik. Endi boshqa kompyuterlaran kirib ko'ramiz ular boshqa serverlarga tushib qolishadi. Xo'sh bu qanday ishladi qanday qilib kira oldik va qanday ishlab turibti?

![nginx-lb](/images/tutorials/web-server/nginx-lb/ip_hash.png)
...
Ushbu holat rasmda tasvirlanganidek ishlaydi. Bu **Weighted Round Robin** load balancing metodida serverlar holatidan kleib chiqib so'rovlarni taqsimlaydi. Bunda `Sserver A` da resurlar ko'pligi va **weight** yuqoriligi sabab ko'proq so'rovlar yuboriladi `Server C` da esa resurslar kamligi va **weight** kichikligi sabab kamroq so'rovlar yuboriladi. Bu Load Balancing server resurlari bir xilm bo'lmaganida qo'l keladi. 

![nginx-lb](/images/tutorials/web-server/nginx-lb/weighted_round_robin.png)

Bizni applicationimiz uchun Round Robin load balabncing metodi to'gri kelmagani uchun ushbu configuratsiyani IP/URL Hash laod balancing metodida ham ishlata olamiz. Quyida IP/URL Hash Load Balancing metodida serverlarga **weight** berib load balancing qilish NGINX konfiguratsiyasi.
...
`upstream` blokiga `least_conn` load balancing metodini qo'shib qo'ydik va endi nginx reload berib sinab ko'ramiz.
...
![nginx-lb](/images/tutorials/web-server/nginx-lb/least_time.png)
