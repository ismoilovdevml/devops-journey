...
Ushbu misolda `pochtaManzili` nomli key-value uchun embedded documentga ega. Ushbu embedded document `Kmanzili`(ko'cha), `shahar`, `davlat` va `pochtaIndeksi` toʻrtta ichki key-value pairidan iborat. Ushbu maxsus embedded document quyida ko'rsatilganidek tuzilgan bo'lishi mumkin.

```json
pochtaManzili: {
    Kmanzili: "Navoiy ko'cha 34",
    shahar: "Toshkent",
    davlat: "O'zbekiston",
    pochtaIndeksi: 10221
},
