...
```bash filename="/etc/systemd/system/prometheus.service" / --web.listen-address=0.0.0.0:9090 \ / / --config.file /etc/prometheus/prometheus.yml \ /
[Unit]
...
```bash filename="/etc/systemd/system/node_exporter.service" /ExecStart=/usr/local/bin/node_exporter/
[Unit]
