...
**3->** Ushbu amaliyotda biz source code orqali Keycloak **23.0.6** versiyasni ubuntu 20.04 serverga o'rnatamiz. Buning uchun keycloak source kodlarini serverimiz `/opt` papkasiga yuklab olamiz va arxivdan chiqaramiz. Rasmiy [Keycloak Downloads](https://www.keycloak.org/downloads.html) sahifasi.

```bash
cd /opt
sudo wget https://github.com/keycloak/keycloak/releases/download/23.0.6/keycloak-23.0.6.tar.gz
```

**4->** Yuklab olgan keycloak source kodlarni .tar arxivini arxivdan chiqarib olamiz. Keycloak source kodlarini arxivdan chiqarib olganimizda source kodlar **keycloak-23.0.6** papkasida bo'ladi biz uni nomini **keycloak**ga o'zgartiramiz. Biz **/opt** papkasi ichidamiz.

```bash
sudo tar -xvzf keycloak-23.0.6.tar.gz
sudo mv keycloak-23.0.6 /opt/keycloak
```
![keycloak-sozlash](/images/article/keycloak-server/keycloak1.png)
