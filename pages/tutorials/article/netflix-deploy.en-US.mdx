...
* [Jenkinsga email(gmail) integratsiya qilish](#jenkinsga-emailgmail-integratsiya-qilish)

* [SonarQubeni Jenkins uchun sozlash](#sonarqubeni-jenkins-uchun-sozlash)

* [Jenkinsga OWASP Dependency Check, JDK va Node.js pluginlarini o'rnatish va sozlash](#jenkinsga-owasp-dependency-check-jdk-va-nodejs-pluginlarini-ornatish-va-sozlash)

* [Jenkinsda birinchi CI pipeline yozish](#jenkinsda-birinchi-ci-pipeline-yozish)

* [Jenkins pipelinega Docker image yaratish va Docker Registryga push qiladigan bosqich qo'shish](#jenkins-pipelinega-docker-image-yaratish-va-docker-registryga-push-qiladigan-bosqich-qoshish)
...
Sonar Qubega domen ulab ssl sertifat olish uchun Jenkinsga domen ulab ssl olganimizdek bir hil, Sonar Qube `9000` portda ishlab turibti Nginx bilan `proxy_pass` reverse-proxy qilib expose qilib domen ulaymiz.

```bash
nano /etc/nginx/sites-available/sonarqube.samidello.uz
```
Ushbu faylga quyidagi Nginx configuratsiyani joylashtiramiz.

```nginx filename="/etc/nginx/sites-available/sonarqube.samidello.uz" /proxy_pass/ /9000/ /sonarqube.samidello.uz/
server {
    listen 80;
    server_name sonarqube.samidello.uz;
...
```
Nginx configuratsiya to'griligini tekshiramiz.
...
```bash /sonarqube.samidello.uz/
sudo ln -s /etc/nginx/sites-available/sonarqube.samidello.uz /etc/nginx/sites-enabled/sonarqube.samidello.uz
```
**Let's Encrypt**, **certboot** yordamida **SSL** sertifikat olamiz

```bash
sudo certbot --nginx -d sonarqube.samidello.uz
```
...
API Keyni nusxalab olib qo'yamiz loyihamizda ishlatamiz. TMDB API KEYni olganimizdan keyin uni Jenkins crdentialsga qo'shib qo'yishimiz kerak.
...
Serverga garafana o'rnatganingizdann keyinnkeling unga domen ulab nginx configuratsiya qilamiz.
...
```bash /grafana/
sudo nano /etc/nginx/sites-available/grfaana.samidello.uz
```

Grafana `3000` portda ishlab turadi bizni reverse-proxy proxy_pass qilib expose qilamiz.

```bash filename="/etc/nginx/sites-available/grafana.samidello.uz" /grafana.samidello.uz/ /3000/
server {
    listen 80;
    server_name grafana.samidello.uz;
...
```bash /grafana.samidello.uz/
sudo ln -s /etc/nginx/sites-available/grafana.samidello.uz /etc/nginx/sites-enabled/grafana.samidello.uz
```
...
```bash
sudo certbot --nginx -d grafana.samidello.uz
```
...
## Jenkinsga OWASP Dependency Check, JDK va Node.js pluginlarini o'rnatish va sozlash
...
SonarQubedan Token geneatsiya qilib olganimzidan keyin uni Jenkins Credentialsga qo'shib qo'yishimiz kerak.
...
* **Kind:** Secret text
* **Secret:** bu yerda SonarQube-dan qolgan tokenimizni joylashtiramiz.
* **ID:** sonar-token

SonarQube uchun token olib Jenkins crdentialsga qo'shganimzidan keyin SonarQube serverni Jenkinsg aulashimiz kerak. 
...
* **Server URL:** - sonarqube.samidello.uz(SonaQube serveringiz manizli)

* **Server authentication token:** - hozir qo'shgan tokenimiz(`Sonar-token`)ni ko'rsatib qo'yamiz shunda Jenkins pipelineda SonarQubega ulana olamiz. `Apply` v a`Save` ni bosib chiqamiz. 
...
SonarQubeda endi loyihamizni analysis qilishi uchun loyihamizni qo'shamiz. SonarQube bosh sahifasiga kirsangiz sizda quyiagi oyna ochilishi kerak siz **Manually** tanlaysiz.
...
Bu yerda loyihamizni nomini kiriamiz rasmda ko'rsatilganidek va **Set Up** bosib keyingi bosqichga o'tamiz.
...
```
`cat netflix >> authorized_keys` Ushbu buyruq `netflix` faylining mazmunini `authorized_keys` faylining oxiriga qo'shadi.
...
```bash /netflix/
sudo nano /etc/nginx/sites-available/netflix.samidello.uz
```

```bash filename="/etc/nginx/sites-available/netflix.samidello.uz" /netflix.samidello.uz/
server {
    listen 80;
    server_name netflix.samidello.uz;
...
```bash /netflix.samidello.uz/
sudo ln -s /etc/nginx/sites-available/netflix.samidello.uz /etc/nginx/sites-enabled/netflix.samidello.uz
```
Nginx configuratsiyamiz to'grilinini tekshiramiz.
...
```bash
sudo certbot --nginx -d netflix.samidello.uz
```
